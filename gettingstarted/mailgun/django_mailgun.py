"""
Taken from here: https://github.com/BradWhittington/django-mailgun

pip gave me the following error when I tried to install through pip:

cbarker$ pipenv install django-mailgun
Courtesy Notice: Pipenv found itself running within a virtual environment, so it will automatically use that environment, instead of creating its own for any project. You can set PIPENV_IGNORE_VIRTUALENVS=1 to force pipenv to ignore that environment and create its own instead. You can set PIPENV_VERBOSITY=-1 to suppress this warning.
Installing django-mailgun...
Error:  An error occurred while installing django-mailgun!
Error text: 
ERROR: Could not find a version that satisfies the requirement django-mailgun (from versions: none)
...
class MailgunBackend(BaseEmailBackend):
    ...
    def __init__(self, fail_silently=False, *args, **kwargs):
        ...
        try:
            ...
            self._server_name = server_name or getattr(
            ...
            if data_to_transform is not None:
                if isinstance(data_to_transform, (list, tuple)):
                    ...
                    for data in data_to_transform:
                        api_data.append(
                            (api_transformer[0], api_transformer[1](data)))
                    ...
                    api_data.append(
                        (api_transformer[0], api_transformer[1](data_to_transform)))
        return api_data
    ...
    def _send(self, email_message):
        ...
        try:
            ...
            if email_message.bcc:
                bcc_recipients = [sanitize_address(
                    addr, email_message.encoding) for addr in email_message.bcc]
                post_data.append(('bcc', (",".join(bcc_recipients)),))
            if email_message.cc:
                cc_recipients = [sanitize_address(
                    addr, email_message.encoding) for addr in email_message.cc]
